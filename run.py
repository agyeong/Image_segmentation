...
# class weight 
dist = {i:(train_Y == i).sum().tolist() for i in range(12)} #  class 개수
 
weights = [1/dist[i] for i in range(12)] # 역수
total_weights = sum(weights) # 합계

class_weight = torch.FloatTensor([w/total_weights for w in weights]).to(device) # total weight로 다시 나눠서 가중치 총합 = 1
criterion = nn.CrossEntropyLoss(weight = class_weight, ignore_index=12) # 손실함수에 적용 / 12 = Void 
...
for epoch in range(num_epoch):
    ...
    for batch, (x, y) in enumerate(tqdm(train_data, total=len(train_data))):
        ...
        corrects = torch.sum((preds == y) & (y != 12)) # except void 
        
        nvoid = int((y==12).sum()) # except void 
    ...
    scheduler.step(loss_running.avg) # Adjust Learning Speed
